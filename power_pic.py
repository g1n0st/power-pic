...
class PowerPICSimulator(FLIPSimulator):
    def __init__(self,
        ...
        free_surface = True):
            ...
            # self.w_p = ti.field(real, shape=self.max_particles)
            self.sum_j = ti.field(real)
            ti.root.dense(self.indices, res).place(self.s_j, self.sum_j)
    ...
    def init_sinkhorn_algo(self):
        ...
        for p, i, j in ti.ndrange(self.total_mk[None], self.R, self.R):
            base = (self.p_x[p] / self.dx).cast(ti.i32)
            x, y = base[0] + (i - self.R_2), base[1] + (j - self.R_2)
            if self.is_valid(ti.Vector([x, y])):
                self.K[p, i, j] = self.K_pj(self.p_x[p], (ti.Vector([x, y]) + 0.5) * self.dx)
            else:
                self.K[p, i, j] = 0
    ...
    def p2g(self):
        ...
        for p, i, j in ti.ndrange(self.total_mk[None], self.R, self.R):
            ...
            x, y = base[0] + (i - self.R_2), base[1] + (j - self.R_2)
            if self.is_valid(ti.Vector([x, y])):
                pos = (ti.Vector([x, y]) + 0.5)
                for k in ti.static(range(self.dim)):
                    utils.splat_w(self.velocity[k], self.velocity_backup[k], self.p_v[p][k], pos - 0.5 * (1 - ti.Vector.unit(self.dim, k)), self.T[p, i, j] / V_p)
